...
summary: 'Splitting a video course platform in two — one signs a short-lived token, one streams from outside the webroot — so videos never get a shareable URL.'
