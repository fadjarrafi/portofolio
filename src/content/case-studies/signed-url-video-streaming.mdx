...
<div className="flex justify-center">
  <img src="/static/images/signed-url-video-streaming.svg" alt="Diagram of the two-phase flow: the browser requests a playback token from the API service, which looks up the file and signs a 60-second HMAC-SHA256 token back to the browser; the browser then requests GET /video/{token} from the web app service, which verifies the signature and expiry and either streams the file from outside the webroot or returns a 403 with nothing served" width="700" />
</div>
