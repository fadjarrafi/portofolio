...
Kalau aplikasimu tiba-tiba jadi lambat begitu mulai menampilkan data asli—bukan lagi sekadar beberapa baris data uji—kemungkinan besar penyebabnya adalah N+1 query problem. Ini salah satu bug performa paling umum di kode backend, dan salah satu yang paling gampang ditulis tanpa disadari—N+1 query problem bersembunyi di dalam kode yang terlihat benar-benar benar.

## Apa itu N+1 Query Problem

N+1 query problem terjadi ketika kode menjalankan satu query untuk mengambil daftar record, lalu menjalankan satu query lagi *per record* untuk mengambil data terkait. Untuk daftar 50 post, artinya 1 query untuk post-nya ditambah 50 query untuk masing-masing author-nya—51 query hanya untuk merender satu halaman.
